...
const CardSchema = z.object({
    ...
    type: z.enum([
        ...
        'slider',
        'range-slider',
    ]).describe('Card type. Determines which fields are required and the shape of the answer value:\n' +
...
export function createMcpServer(store, baseUrl) {
    const server = new McpServer({
        name: 'QuestionPad',
        version: '0.2.0',
        description: 'Collect structured feedback from humans via interactive web forms. ' +
    ...
    server.registerTool('create_session', {
        ...
        description: 'Create a feedback session with one or more participants. Each participant gets their own URL ' +
        ...
        inputSchema: {
            agentId: z.string().optional().describe('Your agent identifier. Omit on first call — one will be generated and returned. ' +
    ...
    server.registerTool('get_sessions', {
        title: 'Get Sessions',
        description: 'Poll for participant answers. Returns the current status and any submitted answers for your sessions.\n\n' +
        ...
        inputSchema: {
            ...
            guids: z.array(z.string()).optional().describe('Specific session GUIDs to fetch. Omit to return all sessions for this agentId.'),
        },
    }, (args) => {
    ...
    server.registerTool('update_session', {
        ...
        description: 'Replace the question cards on a session that hasn\'t been submitted yet. ' +
    ...
    server.registerTool('close_sessions', {
        ...
        description: 'Close sessions and retrieve the final answers. Closed sessions are locked — ' +
        ...
        inputSchema: {
            agentId: z.string().describe('Your agent identifier (returned by create_session).'),
            guids: z.array(z.string()).optional().describe('Specific session GUIDs to close. Omit to close all sessions for this agentId.'),
        },
    }, (args) => {
